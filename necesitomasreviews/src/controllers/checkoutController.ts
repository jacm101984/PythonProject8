...
export const checkoutController = {
  ...
  async createOrder(req: Request, res: Response) {
    try {
      ...
      const order = new Order({
        ...
        promoterId,
        paymentMethod,
        status: paypalOrderId ? OrderStatus.COMPLETED : OrderStatus.PENDING, // Si hay paypalOrderId, el pago ya está completado
        shippingInfo,
        paymentDetails: {
      ...
      if (paypalOrderId) {
        ...
        await mailerService.sendPaymentConfirmationEmail(user.email, {
      ...
      await mailerService.sendOrderConfirmationEmail(user.email, {
        orderId: order._id.toString(),
        planName: plan.name,
        cardCount: plan.cardCount,
        totalAmount
  ...
  async handlePaymentSuccess(req: Request, res: Response) {
    try {
      ...
      if (order.paymentMethod === 'paypal') {
        ...
        paymentId = (payment_id || collection_id) as string;
      } else {
        paymentId = order.paymentDetails.externalId;
      }

      // Verificar el estado del pago
      const { verified, transactionId } = await paymentService.verifyPayment(
        paymentId,
   ...
   */
  async verifyPayPalPayment(req: Request, res: Response) {
    try {
      ...
      const { verified, transactionId } = await paymentService.verifyPayPalOrderDirectly(paypalOrderId);
  ...
  async retryPayment(req: Request, res: Response) {
    try {
      ...
      if (![OrderStatus.PENDING, OrderStatus.FAILED, OrderStatus.CANCELLED].includes(order.status)) {
