// Ejemplo de controlador para tarjetas NFC (src/controllers/nfcController.ts)
import { Request, Response } from 'express';
import NFCCard from '../models/nfcCardModel';
import Scan from '../models/scanModel'; // Modelo para registrar escaneos
import Review from '../models/reviewModel'; // Modelo para registrar reseñas

// Obtener todas las tarjetas del usuario
export const getUserCards = async (req: Request, res: Response) => {
  try {
    const cards = await NFCCard.find({ owner: req.user.id });
...
export const deleteCard = async (req: Request, res: Response) => {
  try {
    const card = await NFCCard.findOneAndDelete({
      _id: req.params.id,
      owner: req.user.id
    });
...
export const getCardStats = async (req: Request, res: Response) => {
  try {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const weekScans = await Scan.countDocuments({
        card: cardId,
        createdAt: {
          $gte: startDate,
          $lte: endDate
      ...
      const weekReviews = await Review.countDocuments({
        card: cardId,
        createdAt: {
          $gte: startDate,
          $lte: endDate
...
export const redirectCard = async (req: Request, res: Response) => {
  try {
    const cardId = req.params.id;
